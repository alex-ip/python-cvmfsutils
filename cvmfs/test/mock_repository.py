...
class CvmfsTestServer(TCPServer):
    allow_reuse_address = True

    def __init__(self, document_root, bind_address, handler):
        self.document_root = document_root
        TCPServer.__init__(self, bind_address, handler)


class CvmfsRequestHandler(SimpleHTTPRequestHandler):
        ...
        with open(new_whitelist, 'wb+') as new_wl:  # TODO: more elegant is Py 2.7
            with open(old_whitelist, 'rb') as old_wl:
                ...
                while True:
                    ...
                    pos = old_wl.tell()
            new_wl.write(b"--\n")
            new_wl.write(wl_hash.hexdigest().encode('utf8'))
            new_wl.write(b"\n")
            key = RSA.load_key(self.master_key)
            sig = key.private_encrypt(wl_hash.hexdigest().encode('utf8'), RSA.pkcs1_padding)
            new_wl.write(sig)
        os.rename(new_whitelist, old_whitelist)

    def _setup_repository(self):
        self.sandbox.create_directory(self._extract_dir)
        repo = BytesIO(base64.b64decode(MockRepository.repo_data))
        repo_tar = tarfile.open(None, "r:gz", repo)
        repo_tar.extractall(self._extract_dir)
        pubkey = self.sandbox.write_to_temporary(MockRepository.repo_pubkey.encode('utf8'))
        self.public_key = pubkey
        privkey = self.sandbox.write_to_temporary(MockRepository.repo_privkey.encode('utf8'))
        self.private_key = privkey
        mkey = self.sandbox.write_to_temporary(MockRepository.repo_masterkey.encode('utf8'))
